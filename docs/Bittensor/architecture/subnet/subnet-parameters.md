...
* **registration\_allowed**: When a subnet updates or there is an issue - the subnet owner may pause registrations to allow for all miners and validators to update their servers without the risk of deregistration.
...
* **min\_burn/max\_burn**: Node registration costs (in rao - 1e-9 tao).  The max listed above is 100 tao.
