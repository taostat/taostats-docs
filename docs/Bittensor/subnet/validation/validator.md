...
In order to be recognised as a validator the participant must and on-chain verification of the ownership of their key using the [Bittensor Delegates](https://github.com/opentensor/bittensor-delegates/) repository. The full list of all keys and the associated metadata they submitted can be seen on a [public delegates.json file](https://github.com/opentensor/bittensor-delegates/blob/main/public/delegates.json) which is used for labelling in the explorer and recognition of validators on Taostats.

The full list of these validators can be viewed on the Verified Validators page with more information available in the [Validators](doc:validators) section of the documentation.
