...
Technical Explainer â€” Based on subtensor PR #2658

## 1. What Is Conviction?

Conviction is Bittensor's governance mechanism that ties voting power to long-term commitment. When you lock alpha tokens to a subnet, you earn **conviction** â€” a measure of your stake's weight in governance decisions and subnet ownership.
...
* **One lock per coldkey per subnet** â€” You can only have a single active lock targeting one hotkey on each subnet
  * **Lock to a hotkey** â€” Your locked alpha is associated with a specific hotkey on that subnet. This is typically the subnet owner's hotkey, or the hotkey of someone building conviction to challenge for subnet ownership.
    * **Top-ups must match** â€” Additional locks to the same subnet must target the same hotkey; you can't split locks across multiple hotkeys
      * **Perpetual by default** â€” Locked mass stays constant indefinitely, allowing conviction to grow toward 100%
        * **Optional decay** â€” Use `set_perpetual_lock(false)` to enable exponential decay if you want to eventually unstake
          <br />
          <br />
...
* `locked_mass` remains constant forever
  * `conviction` grows exponentially from 0 toward `locked_mass`
    * Conviction asymptotically approaches 99% after 1 year, 99.9%+ after 2 years
      * Cannot unstake â€” perpetual locks stay locked unless you toggle to decaying mode
        <br />
...
* **Instant conviction** â€” `conviction` equals `locked_mass` immediately (no growth period)
  * **Auto-locked owner cut** â€” The owner's share of emissions is automatically locked each block, building conviction continuously
...
* **Owner cut auto-locked:** Each block, the owner's emissions share is locked to their subnet ownership hotkey
  * **No initial alpha distribution:** In v2, subnet registration does not distribute initial alpha to the owner. Conviction builds solely through the auto-locked owner cut from ongoing emissions.
    * **Instant conviction:** Unlike validators, owners don't wait for conviction to grow â€” `conviction = locked_mass` immediately
...
| ------------------------------------ | ------------------------- | --------- | --------------------------------------------------------------------------------------------------------------- |
| `Lock`                               | (coldkey, netuid)         | LockState | Legacy/compatibility â€” tracks basic lock state                                                                |
...
* `lock_stake(hotkey, netuid, amount)` â€” Lock alpha to a hotkey on a subnet (perpetual by default)
  * `set_perpetual_lock(netuid, perpetual: bool)` â€” Toggle between perpetual (true) and decaying (false) mode
