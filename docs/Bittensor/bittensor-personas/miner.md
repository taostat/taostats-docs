...
You'll need to [register a node](doc:node-registration) with a Bittensor hotkey, and use this same hotkey in your server.

# Ranking

Each miner's responses are graded by validators. The validators then weigh the responses of all miners and rank them via weights. The weights are posted to [Yuma Consensus](doc:consensus) to create an incentive score - and the higher the incentive, the higher the reward.
