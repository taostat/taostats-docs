---
title: The three stages of a multisig operation
excerpt: ''
deprecated: false
hidden: false
metadata:
...
> 📘 **New to multisig?**
>
> For a step-by-step walkthrough see the [Multisig tutorial](/docs/multisig-tutorial), and for real flows decoded from the chain see [Multisig worked examples](/docs/multisig-examples). This page covers the underlying extrinsics.
...
* **Collect signatures** In this step, subsequent calls are collected to reach the threshold.  These can be `Multisig.as_multi` or `Multisig.approve_as_multi` calls.
  * These transactions contain a `maybeTimepoint` argument with two parameters to connect with the initial multisig call.
