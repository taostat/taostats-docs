...
* At launch, a maximum of 2 subnet mechanisms can be implemented, with the ceiling rising to up to 8 mechanisms over time.
...
* Subnet emission to validators will be based on Vtrust and Dividends in each subnet mechanism, multiplied by the mechanism distribution.

Each subnet reports its mechanism configuration through the [Get Subnets](/api-reference/subnet/get-subnet-latest) endpoint, via the `sub_subnet_count` (number of mechanisms) and `sub_subnet_emission_split` (per-mechanism emission distribution) fields.

## Mechanism architecture

Subnets can have 256 [neurons](/docs/glossary#neurons) (netuids 0 and 1 are exceptions to this rule), set in the subnet [hyperparameters]().  In time, this will be a custom configurable value.
...
1. **Validation of miner output**.  This is typically done by sending regular requests to each miner and then assigning a value/score to the response.  These scores are usually added to a moving average of the miners performance which enables a score (weights) to be set at regular intervals on the blockchain for all miners by that validator. These weights form part of the incentive landscape which when combined with the weights of the other validators using [Yuma Consensus](/docs/consensus) are then used to define and distribute emissions.
...
* [Validator (Architecture)](/docs/validation)
* [Validator (Persona)](/docs/validator)
* [Emissions for Validators](/docs/dividends-for-validators)
...
Miners are ranked by validators and given an incentive score.  Miners with higher incentive values receive higher [emissions](/docs/glossary#emissions).

* [Miner (Architecture)](/docs/mining)
* [Miner (Persona)](/docs/miner)
* [Emission for Miners](/docs/consensus-for-miners)

### Consensus

Each subnet undertakes a specific task. In order to evaluate how the task is being performed, an incentive mechanism is used by validators to evaluate work performed by the miners.  The validators score each miner, and set weights on-chain each epoch. These weights are aggregated by [Yuma Consensus](/docs/consensus) to form an overall incentive landscape upon which trust values are calculated to determine emissions.
...
When a new subnet is registered, its [Subnet Pool](/docs/subnet-pools) is seeded from the registration cost: a portion of the lock cost is paid into the pool as TAO, and a matching amount of alpha is minted to set the opening price. The remainder of the lock cost is recycled.

## Registration Cost

There is a cost to register a new subnet.  The cost for a new registration is based on the demand of subnets - more frequent registrations raise the cost, and then the cost goes down over time. A portion of the lock cost is paid into the [Subnet Pools](/docs/subnet-pools). The remainder is recycled.

You can determine the current price using the [Bittensor CLI](/docs/command-line-tool):
...
<Image border={false} alt="A screenshot of the Subnet registration cost over time." src="https://raw.githubusercontent.com/taostat/taostats-docs/v5.0/docs-assets/img/64e1106e5639712f.jpg" />
...
See [Subnet Emission tao and alpha](/docs/subnet-emissions)

# Subnet de-registration

In September 2025, subnet deregistration was re-added.  The subnet with the lowest moving average price will be deregistered when a new subnet is created.
