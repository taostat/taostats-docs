...
Miner collateral is a refundable-by-work registration bond for Bittensor subnets. Part of your registration fee stops being recycled and instead becomes locked stake you can only recover by earning emission — or by the subnet dissolving. It is **live on Finney mainnet at spec 437**.
...
Registration on a busy subnet already costs real TAO — but by default 100% of that cost is recycled (swapped to alpha, and that alpha removed from supply; the chain labels this the "burn"). Once you're registered, the value is gone to you whether you mine honestly or squat the slot doing nothing.
...
The drain settles against **full hotkey emission** — miner incentive *and* validator dividends — releasing the lock at rate `k`.
...
## What this changes for reading stake: available ≠ total
...
The chain computes this internally (`available_to_unstake_from_hotkey` does `stake − collateral`, stacking with conviction locks). Storage and events to read: `MinerCollateral`, `ColdkeyMinerCollateral`, `CollateralLockShare` (p), `CollateralDrainRatio` (k); events `CollateralLocked`, `CollateralLockShareSet`, `MinCollateralSet`; plus lineage (`ColdkeyCollateralHotkeys`, `HotkeySuccessor` / `HotkeyRoot`, `ColdkeySuccessor` / `ColdkeyRoot`).
...
3. You recover it by earning emission: the drain settles against **full hotkey emission** — miner incentive and validator dividends — releasing the lock back to free stake at rate `k`.
...
8. When reading a balance: track `available = total − collateral − conviction lock`, or balances will read too high.

> 📘 **See also**
>
> This mechanic first landed as a tracked runtime change — see the [Miner collateral protocol-change page](/docs/miner-collateral) for the release-train history and source PRs. For how it interacts with the base registration flow, see [Neuron registration](/docs/node-registration).
